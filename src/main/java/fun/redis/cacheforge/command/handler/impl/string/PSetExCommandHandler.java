 ...
 */
@Slf4j
public class PSetExCommandHandler implements WriteCommandHandler {
	...
	public void handle(ChannelHandlerContext ctx, Command command) {
		try {
			...
			if (args.length == 3) {
				...
				String value = args[2];
				CommandRegistry.get("set").handle(ctx, new Command("set", new String[]{key, value, "PX", String.valueOf(expireTime)}));
			} else {
				log.error("psetex命令参数错误");
				// todo
				ctx.writeAndFlush(toErrorMessage(Err.ERR));
			}
		} catch (Exception e) {
			log.error("psetex命令异常");
			// todo
			ctx.writeAndFlush(toErrorMessage(Err.ERR));
		}
	}
