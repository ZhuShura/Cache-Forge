...
@Slf4j
public class SMIsMemberCommandHandler implements ReadCommandHandler {
    @Override
    public void handle(ChannelHandlerContext ctx, Command command) {
        try {
            ...
            if (args.length >= 2) {
                String key = args[0];
                List<String> members = List.of(args).subList(1, args.length);
                ...
                for (String member : members) {
                    result.add(set.contains(member) ? 1 : 0);
                }
